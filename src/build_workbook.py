...
def build_summary(wb, RAW, S):
    ...
    sections = [
        ("1. Capital structure & financing strategy — who is financed most conservatively, and has that changed?", [
            "BOTTOM LINE: Costco is the most conservatively financed of the group and has become more so every year since FY2021. Walmart's leverage is unchanged over the decade. Kroger has nearly tripled its debt-to-equity since FY2023 to fund share repurchases after the Albertsons merger collapsed – a deliberate capital-return decision, but the one red flag in the set.",
            f'="Costco is financed most conservatively and is still de-levering: debt-to-equity fell from "&{x2(R_(2016, "DE_Costco"))}&" in FY2016 to "&{x2(R_(2025, "DE_Costco"))}&" in FY2025. Total debt has stayed near "&{bn(V_(2025, "Costco_TotalDebt"))}&" while equity compounded from "&{bn(V_(2016, "Costco_TotalEquity"))}&" to "&{bn(V_(2025, "Costco_TotalEquity"))}&" – growth and buybacks are funded from operating cash flow, not borrowing."',
            f'="Walmart is stable by design: debt-to-equity has held in a "&{x2("MIN(DE_Walmart)")}&"–"&{x2("MAX(DE_Walmart)")}&" band for the whole decade. Total debt in FY2025 ("&{bn(V_(2025, "Walmart_TotalDebt"))}&") is only "&{pct(f"{V_(2025, 'Walmart_TotalDebt')}/{V_(2016, 'Walmart_TotalDebt')}-1")}&" above FY2016 ("&{bn(V_(2016, "Walmart_TotalDebt"))}&") on revenue that is "&{pct(f"{V_(2025, 'Walmart_Revenue')}/{V_(2016, 'Walmart_Revenue')}-1")}&" larger, so leverage has fallen relative to the size of the business."',
            f'="Kroger is the outlier and the one genuine red flag. It de-levered from "&{x2(R_(2016, "DE_Kroger"))}&" (FY2016) to "&{x2(R_(2023, "DE_Kroger"))}&" (FY2023), then re-levered to "&{x2(R_(2024, "DE_Kroger"))}&" in FY2024 and "&{x2(R_(2025, "DE_Kroger"))}&" in FY2025. The FY2024 10-K shows "&{bn(kr_debt)}&" of long-term debt issued for the Albertsons merger (terminated December 2024) and "&{bn(kr_bb24)}&" of share repurchases, followed by a further "&{bn(kr_bb25)}&" of repurchases in FY2025; equity fell from "&{bn(V_(2023, "Kroger_TotalEquity"))}&" to "&{bn(V_(2025, "Kroger_TotalEquity"))}&" in two years."',
            '="Reading: Kroger\'s leverage is funding buybacks, not growth – revenue has been flat since FY2022 – and it is doing so on an equity base shrunk by charges. That is a deliberate capital-return choice, but it leaves the least financial flexibility in the group and would be the first balance sheet to strain if margins slip further."',
            f'="Lease-adjusted view (ASC 842): including operating lease liabilities lifts every ratio but changes no ranking – FY2025 debt-to-equity incl. leases is "&{x2(R_(2025, "DEL_Costco"))}&" Costco, "&{x2(R_(2025, "DEL_Walmart"))}&" Walmart, "&{x2(R_(2025, "DEL_Kroger"))}&" Kroger."',
        ]),
        ("2. Resilience through 2020–2023 — who absorbed COVID and the inflation spike best, and who took the biggest hit?", [
            "BOTTOM LINE: Costco absorbed the shock best – revenue, net margin and liquidity all rose through the window. Walmart took the biggest hit, with FY2022 gross and operating margins the lowest of the decade, but recovered within two years. Kroger's margin swings were fuel-mix effects, not financial stress.",
            f'="Costco absorbed the shock best. Revenue grew "&{pct(R_(2020, "GR_Costco"))}&", "&{pct(R_(2021, "GR_Costco"))}&" and "&{pct(R_(2022, "GR_Costco"))}&" in FY2020–FY2022; net margin rose from "&{pct(R_(2019, "NM_Costco"))}&" (FY2019) to "&{pct(R_(2023, "NM_Costco"))}&" (FY2023); the current ratio peaked at "&{x2(R_(2020, "CR_Costco"))}&" in FY2020 and never fell below "&{x2("MIN(CR_Costco)")}&". Gross margin dipped to "&{pct(R_(2022, "GM_Costco"))}&" in FY2022 – inflation absorbed on members\' behalf – and has since recovered to "&{pct(R_(2025, "GM_Costco"))}&"."',
            f'="Walmart took the biggest single-year hit, in FY2022 (year ended January 2023): gross margin fell to "&{pct(R_(2022, "GM_Walmart"))}&" and operating margin to "&{pct(R_(2022, "OM_Walmart"))}&", the lowest of the decade, as inventory built and was marked down; the current ratio fell from "&{x2(R_(2020, "CR_Walmart"))}&" (FY2020) to "&{x2(R_(2022, "CR_Walmart"))}&". Recovery was quick – operating margin was back to "&{pct(R_(2024, "OM_Walmart"))}&" by FY2024 and net margin to "&{pct(R_(2025, "NM_Walmart"))}&" in FY2025."',
            f'="Kroger\'s margins moved most but for a mix reason: gross margin jumped to "&{pct(R_(2020, "GM_Kroger"))}&" in FY2020 when low-margin fuel sales collapsed, then fell to "&{pct(R_(2022, "GM_Kroger"))}&" in FY2022 as fuel prices spiked. Through FY2020–FY2023 net margin stayed between "&{pct(f"MIN({W('NM_Kroger')})")}&" and "&{pct(f"MAX({W('NM_Kroger')})")}&", and the current ratio ("&{x2(f"MIN({W('CR_Kroger')})")}&"–"&{x2(f"MAX({W('CR_Kroger')})")}&") is structurally the lowest of the three – a feature of its working-capital model, not a shock effect."',
            f'="Sam\'s Club (segment) grew net sales "&{pct(R_(2020, "GR_SamsClub"))}&", "&{pct(R_(2021, "GR_SamsClub"))}&" and "&{pct(R_(2022, "GR_SamsClub"))}&" over FY2020–FY2022 with operating margin between "&{pct(f"MIN({W('OM_SamsClub')})")}&" and "&{pct(f"MAX({W('OM_SamsClub')})")}&" through FY2020–FY2023 – the club format was the clear beneficiary of pandemic-era bulk buying and inflation trade-down."',
        ]),
        ...
        ("Notable trend shifts since FY2016", [
            f'="Costco\'s step-change: revenue growth of "&{pct(R_(2021, "GR_Costco"))}&" and "&{pct(R_(2022, "GR_Costco"))}&" in FY2021–FY2022 lifted it from "&{bn(V_(2016, "Costco_Revenue"))}&" to "&{bn(V_(2025, "Costco_Revenue"))}&" – it is now "&TEXT({V_(2025, "Costco_Revenue")}/{V_(2025, "Kroger_Revenue")},"0.0")&"x Kroger\'s size, versus "&TEXT({V_(2016, "Costco_Revenue")}/{V_(2016, "Kroger_Revenue")},"0.0")&"x in FY2016."',
            f'="Kroger\'s reversal: five years of de-leveraging (FY2018–FY2023) undone in two. Debt-to-equity "&{x2(R_(2023, "DE_Kroger"))}&" → "&{x2(R_(2025, "DE_Kroger"))}&"; revenue "&{pct(R_(2024, "GR_Kroger"))}&" and "&{pct(R_(2025, "GR_Kroger"))}&" in FY2024–FY2025."',
            '="ASC 842 (FY2019; Costco FY2020) put operating leases on every balance sheet. Total debt in this model excludes them so FY2016–FY2018 remain comparable; the supplementary lease-inclusive ratio shows the effect."',
        ]),
        ("Data caveats", [
